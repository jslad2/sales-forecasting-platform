        ...
        try:
            ...
            if st.button("Start Forecast"):
                ...
                try:
                    ...
                    # 🔍 **Debugging Step: Data Inspection**
                    st.write(f"📊 x_train Shape: {x_train.shape}")
                    st.write(f"📊 y_train Shape: {y_train.shape}")
                    st.dataframe(x_train.head())  # Show sample data
                    st.dataframe(y_train.head())  # Show sample target values

                    # ✅ Check for NaNs or Inf values before training
                    ...
                    if len(x_train) <= 5:
                        st.error("❌ Insufficient data to train AutoML. Need more samples.")
                    ...
                    if not callable(getattr(automl_model, "fit", None)):
                        ...
                        raise ValueError("AutoML.fit() is not callable")
